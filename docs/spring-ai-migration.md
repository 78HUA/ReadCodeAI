...
- ⚠️ **「轮次」这一列当时口径不一致（2026-09-26 补记更正）**：Spring AI 侧只统计"有工具调用的轮次"，
  手写侧统计的是**全部模型轮次**（结论轮、格式重发轮都算）—— 所以 3.67 vs 7.00 **不能直接比**。
  已在引擎侧补记（结论轮与重发轮计入轮次），但**本表数字未重跑**；阅读时以召回为准，轮次/ token 两列只看趋势；
