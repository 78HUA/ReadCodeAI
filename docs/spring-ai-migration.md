...
## 如果合并：简历与技术栈这样改（措辞已备好，等"合并/推送"定了再搬）

> **前提**：主分支现在还没有 Spring AI（默认引擎的变化只在这条分支上）。
> 简历应当描述**主干真实的样子** —— 所以这份措辞等合并（或至少推送）之后再往简历上搬。

**第 1 条（方案 B：手写与框架两张牌都留）**

> 针对链式问题单次检索答不全，手写 ReAct 多跳循环让模型自主决定调哪个工具、跳几跳、何时收尾，
> 并以环检测与四维预算框住循环；另做过 **Spring AI** 引擎的迁移与 A/B 实测（同题同语料召回持平、
> token 少 40%）；实测链式题召回从单跳上限 24.4% 提升到 86.7%。

（备选方案 A「框架当主角」：把"手写"换成"基于 Spring AI 工具循环实现多跳 Agent（环检测/四维预算/
证据核验为自研扩展）"——代价是丢掉"手写 577 行"那个叙事。）

**技术栈药丸**：加 `Spring AI`，撤掉 `LLM接入`（已被覆盖），仍是 9 个：

```
SpringBoot · JavaParser · MySQL · Vue · Redis · RabbitMQ · ReAct多跳 · Tool Calling · Spring AI
```

**合并前建议先做**：推送分支 → 看 CI 是否绿（离线集 206 个）→ 再合并到 `main`；
同时把本页的 A/B 数字按项目惯例补进 `docs/verification-log.md`。
