...
**Q5「菜品分页查询是怎么实现的？」答错了。**
正确答案是 `DishServiceImpl.pageByDish`（第 67 行）/ `DishController.pageByDish`（第 42 行），
但答案引用的是 `OrdersServiceImpl` / `OrderDetailServiceImpl`。

**根因（值得记住）**：问题用**中文**描述，而这几个方法的代码里是**英文标识符**
（`pageByDish`、`Page<Dish>`）。中文 token「分页」「查询」只能匹配到**注释里恰好有这些中文词**的片段 ——
订单实现里有这样的注释，菜品实现里没有。**于是检索把「订单分页」当成了「菜品分页」。**

→ 这正是设计文档里**第 3 层（向量检索）存在的理由**：中文提问与英文标识符之间的鸿沟，
靠全文检索跨不过去。短期可做的缓解是把问题里的中文词也拿去匹配**符号名**
（`菜品` ↔ `Dish` 需要词典或语义，本质上还是第 3 层的活）。

### 尚未完成的部分

- [ ] **10 个问题的逐条人工判定**（`notes/qa-sampling.md`）—— 第 2 步的正式判据，需要人来判
